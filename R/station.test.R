...
#' @import survival
#' @usage station.test(a, v, delta, print.out = FALSE, digits = 3L)
...
#' @seealso \code{\link{coxph.lb}}, \code{\link{coxphlb.ftest}}, \code{\link{coxphlb.phtest}}, \code{\link{station.test.plot}}
...
#' stest1 <- station.test(ExampleData1$a, ExampleData1$y-ExampleData1$a, ExampleData1$delta, print.out=TRUE)
#' print(stest1) 			# display the results
#'
#' stest2 <- station.test(ExampleData2$a, ExampleData2$y-ExampleData2$a, ExampleData2$delta, print.out=TRUE)
#' print(stest2)   		# display the results
#' @export

station.test <- function(a, v, delta, print.out = FALSE, digits = 3L)
{
  len<-length(delta)
  ...
  if (print.out==TRUE) {
  cat("\ttest.statistic = ", format(round(test.statistic, digits)),
      ", p.value = ", format.pval(p.value, digits, eps = 1e-03), "\n", sep = "")}
  out=list(test.statistic = test.statistic, p.value = p.value)
}
